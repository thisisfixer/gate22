...
// Base URL for mock API - dynamically determined
const getApiBaseUrl = () => {
...
export const handlers = [
  ...
  http.get('*/api/auth/me', async ({ request }) => {
    ...
    return HttpResponse.json({
      user: {
        ...
        lastName: 'User',
        username: 'demo',
        pictureUrl: 'https://ui-avatars.com/api/?name=Demo+User&background=random',
      },
      org: {
        orgId: 'org-demo-001',
        orgName: 'Demo Organization',
  ...
  http.get('*/v1/organizations/users', async ({ request }) => {
    await delay(MOCK_DELAY);
    // Get orgId from header
    const orgId = request.headers.get('X-ACI-ORG-ID');
  ...
  http.delete('*/v1/organizations/users/:userId', async ({ params, request }) => {
    await delay(MOCK_DELAY);
    const orgId = request.headers.get('X-ACI-ORG-ID');
